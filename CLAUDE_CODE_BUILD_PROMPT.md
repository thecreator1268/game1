...
5. **Update (implemented):** the accuracy comparison in rules 1–2 is now fed by a Bayesian Knowledge Tracing mastery estimate per patient per domain (`/src/engine/bkt.ts`, table `masteryEstimates`): level up when the estimate is ≥ 0.80 and the game's own last 5 sessions average ≥ 80% (with the response-time trend), level down when it is < 0.40 or on 2 consecutive sessions of rising error rate. The window, thresholds and the logged reason strings ("leveled up: domain mastery estimate 0.91 (at least 0.80) and this game's own last 5 sessions averaged 84% (at least 80%); avg 3.2s") keep their shape. BKT's parameters are literature-typical priors, not fitted values; calibrating them from real patient data is the named next step. No trained model, and no dataset, is needed or claimed.
