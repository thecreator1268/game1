...
The visual direction for this project is already decided and **locked**. Three project skills
override everything below wherever they conflict: `design-system` ("Indopendence" palette, Unbounded +
Space Grotesk, flat card / speech-tag / blob-mascot / black-pill vocabulary), `accessibility-checklist`,
and `motion-system`. This skill's brief to "take aesthetic risk" and "choose typefaces deliberately"
does **not** apply here: the choices were made, so use it only to execute them better (spacing,
hierarchy, type scale within the existing fonts, composition, empty and error states).

- Do not propose a new palette, typeface, hero treatment, or component style. The tokens live in
  `src/index.css` (`--color-*`, `--domain-*`).
- 64px minimum tap targets and 18px body text at every breakpoint; never tighten below them.
- No emoji. Icons come only from the shared sprite (`src/components/IconSprite.tsx`).
- No timers that fail the patient, leaderboards, coins, or chat.
- Report anything broad (restyling a screen, new component) before applying it. Small fixes for a
  named gap can be applied directly using the existing patterns.
