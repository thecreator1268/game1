...
SmritiSetu is a dashboard / multi-step product UI (this skill's own header
says it's not primarily meant for that) — treat every rule below as
**advisory quality-bar guidance** (hover/active states, inline error states,
loading/empty states, one-accent-color discipline, contrast checks), not as
license to reach for this skill's own landing-page defaults. Three **locked**
skills override this one wherever they conflict: `accessibility-checklist`,
`design-system` ("Indopendence"), `motion-system`.

- Do not introduce Tailwind v4, `next/font`, Motion/Framer, GSAP, Radix,
  shadcn, Phosphor/Hugeicons/Tabler icons, or any other library this skill
  recommends by default — this project is Tailwind v3 + the `motion`
  library already wired per `motion-system`, custom SVG sprite icons only,
  no new dependency without checking `package.json` first (Section 3.F of
  this skill already says this — it applies doubly hard here).
- Color/type are fixed to the existing tokens and Unbounded/Space Grotesk —
  ignore Section 4.1/4.2's font and palette rotation guidance entirely.
- 64px minimum tap targets always (accessibility-checklist), even where
  this skill's `VISUAL_DENSITY` dial would suggest tightening.
- Emoji policy: **never**, no override — accessibility-checklist is
  stricter than this skill's own "allow if playful" exception.
- The useful parts of this skill for this project are Section 4.5
  (Interactive UI States: loading/empty/error states, button contrast,
  form contrast) and Section 4.6 (form patterns: label above input, error
  below input) — apply those against the existing flat-card/pill-button
