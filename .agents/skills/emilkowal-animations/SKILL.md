...
## SmritiSetu project scope (read first, every invocation)

`accessibility-checklist`, `design-system` and `motion-system` override this skill wherever they
conflict — it refines execution quality within those, it doesn't propose new colors, fonts, or
override the calm/non-alarming Patient Mode rules (no bounce/overshoot outside the Tier-2 spots
`motion-system` names).

- The shared timing tokens already exist (`--motion-fast` 120ms / `--motion-medium` 240ms /
  `--motion-slow` 650ms, `--ease-standard`, `src/lib/motionTokens.ts`) — use this skill's tables to
  validate those choices against Emil Kowalski's own numbers, don't introduce a second set of
  duration constants alongside them.
- `.tap-press` already implements `transform-scale-097`-style press feedback at `scale(0.96)`
  (chosen to match this app's existing tap-target sizing, not 0.97) and now carries the generic
  `transition` property (2026 motion pass) — reuse it; don't add a second per-component
  `whileTap`/active-scale implementation.
- This project is Tailwind **v3**, not v4 — the `tw-` category (category 8) doesn't apply; treat
  the raw-CSS and Framer Motion (`motion/react`) rules as the source of truth, per this skill's own
  note under that section.
- Ignore any hex/color values in this skill's own examples — colors and fonts come from
  `src/index.css` tokens and the design-system skill, always.
- `interact-*` (drag/gesture) rules are largely N/A outside Dinacharya Sequence and Naksha Jodo,
  the two drag-based games — see `motion-system`'s forgiving-drag notes for those; don't add
  gesture/momentum interactions anywhere else.
- `strategy-keyboard-no-animate` and the reduced-motion rules (category 7) match this project's
  existing `usePrefersReducedMotion` convention (skip to final state, not a shortened version) —
  apply them as confirmation, not a new pattern to introduce.
