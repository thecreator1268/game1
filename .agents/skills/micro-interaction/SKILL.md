...
- Press feedback already exists (`.tap-press`, 0.96 scale, 120ms). Reuse it; don't add per-component
  `whileTap`/`whileHover` variants. No hover-only affordances: patients use touch.
- Success/toast/modal motion follows the existing patterns (`RoundFeedback`, `Modal`, `.toast-in`).
  Use this skill to *polish* them, not replace them.
- Colors and fonts come from `src/index.css` tokens. Ignore the blue focus-ring example
  (`rgba(59,130,246,.5)`) and any hex in the recipes.
- Reduced motion → instant final state, not a shortened fade (see `accessible-animation` scope note).
- No layout animation (`layout`/`layoutId`/`Reorder`) on Dinacharya Sequence, Naksha Jodo or any
  drag game; those screens are high-stakes and must stay predictable.
- The standalone-HTML "Deliver & verify" sections don't apply; verify in the running app.
