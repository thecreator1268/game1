...
`accessibility-checklist`, `design-system` and `motion-system` override this skill wherever they conflict.

- **Reduced motion means instant, not "softened".** SmritiSetu's rule is: `prefers-reduced-motion` →
  skip straight to the final state (no fade, no shortened tier-2 transition). Ignore this skill's
  "tier, don't nuke" advice and its 150ms fallback fades. Use the existing hook
  `src/hooks/usePrefersReducedMotion.ts` (or the lazy `matchMedia` read in `useCountUp`); don't add
  a second `useReducedMotion` hook. Keep the `0.01ms` (never `0s`) trick for any CSS backstop.
- A GSAP timeline must be gated with `gsap.matchMedia()` or an early return that jumps to the end
  state (`tl.progress(1)`), never a shorter version of the same motion.
- Never let an animation block the next tap: no `mode="wait"` gates, no `pointer-events: none`
  during a transition, always skippable.
- Nothing decorative on timer-sensitive or high-stakes patient screens (game rounds, reminder
  confirmation, PIN entry, delete confirmations).
- The "Deliver & verify (standalone HTML)" sections describe a different deliverable; here the
  work lives in the React app. Verify with the running dev server, in both normal and
  reduced-motion (Playwright `browser_emulate_media` with `reducedMotion: 'reduce'`).
