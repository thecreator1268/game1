...
- **This app is React Router v7 (Vite SPA), not Next.js.** Route transitions are already built:
  `src/components/RouteTransition.tsx` (220ms fade + 14px slide, direction from
  `useNavigationType()`), specified in `motion-system` Tier 1 #2. Do not port `template.tsx`,
  `FrozenRouter`, or `next-view-transitions` (Next internals, meaningless here).
- Do not use `mode="wait"`: it delays the incoming screen behind the exit and would gate the
  patient's next tap. Keep exits absent (entrance-only) as the existing spec says.
- View Transitions API: only as a possible enhancement to the caregiver-only `variant="dynamic"`,
  never in Patient Mode, and always behind `document.startViewTransition` + reduced-motion checks.
- Reduced motion → instant swap (no crossfade fallback).
- Use this skill only to debug or tune the existing `RouteTransition`.
